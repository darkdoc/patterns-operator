...
podman-build-amd64: buildah-manifest ## build the container in amd64
	...
	buildah bud --platform linux/amd64 --format docker -f Dockerfile -t "${OPERATOR_IMG}-amd64"
...
podman-build-arm64: buildah-manifest ## build the container in arm64
	...
	buildah bud --platform linux/arm64 --build-arg GOARCH="arm64" --format docker -f Dockerfile -t "${OPERATOR_IMG}-arm64"
